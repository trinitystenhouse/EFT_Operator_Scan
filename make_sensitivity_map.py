...
DRAW_UNITARITY = True   # set False to omit the partial-wave unitarity guide
                        # (the shaded wedge then becomes kinematic-only)
BOUNDARY_SUFFIX = ""
...
def _unitarity_curve(operator: str, mchi_arr: np.ndarray) -> tuple[np.ndarray | None, str | None]:
    ...
    if operator in ("anapole", "charge_radius", "scalar_rayleigh"):
        # scalar_rayleigh is dimension-6, so it takes the dim-6 form. It must be
        # matched BEFORE the "rayleigh" substring test below, which is dim-7.
...
def plot_sensitivity_panel(ax, operator_key: str, halo_profile: str,
    ...
    # CORRECTED 2026-08-07: this read c^{2} for every panel, which is false for
    # the dipoles.  The rescaling actually applied is c^{-coefficient_power},
    # and coefficient_power is 1/n with n = d-4: 1/1 for the dim-5 dipoles,
    # 1/2 for the dim-6 scalar Rayleigh, 1/3 for the dim-7 family.  The dipoles
    # carry c^4 in tau (two insertions, mu = 2c/Lambda) rather than the c^2 of
    # the single-insertion Rayleigh operators, so no single c^q covers all six
    # panels -- c^{1/n} does, and it is the form the Fig. 2 caption already uses.
