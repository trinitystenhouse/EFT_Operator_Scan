...
from core.spectral_reshaping import (  # noqa: E402
    ReshapingConfig,
    clip_boundary_to_removal_support,
...
def unitarity_lambda_curve(operator: str, mchi_grid: np.ndarray) -> np.ndarray:
    ...
    # scalar_rayleigh is Lambda^-2 (lambda_power_in_cross_section=4) and must
    # NOT fall through to this dimension-7 branch by substring match.
    if str(operator) == "scalar_rayleigh":
        return (16.0 * np.pi * xgrid**2) ** 0.25
...
def save_boundary(
    *,
    ...
    np.savez_compressed(
        ...
        roi_emissivity_power=np.float32(args.emissivity_power),
        sigma_model=str(args.sigma_model),
        kernel_nodes=str(args.kernel_nodes),
        # Bin geometry of the fitted spectrum. Recorded since 2026-09-18: before
        # that f_bin was a hard-coded 0.231 (the halo value) for every dataset
        # and no output said which f had been used.
        f_bin=np.asarray(
            args.bin_geometry["f_bin"] if args.f_bin is None
            else np.full_like(args.bin_geometry["f_bin"], float(args.f_bin)),
            dtype=np.float32),
        f_bin_mode=("derived" if args.f_bin is None else "override"),
        f_bin_median=np.float32(args.bin_geometry["f_bin_median"] if args.f_bin is None
                                else float(args.f_bin)),
        bin_ratio_median=np.float32(args.bin_geometry["bin_ratio_median"]),
        bin_ratio_min=np.float32(args.bin_geometry["bin_ratio_min"]),
        bin_ratio_max=np.float32(args.bin_geometry["bin_ratio_max"]),
        bin_grid_uniform=bool(args.bin_geometry["uniform"]),
    )
...
def parse_args():
    ...
    # --- Numerics of the redistribution kernel and the removal cross section.
    # Defaults reproduce every published grid byte for byte; the alternatives
    # are the corrected paths (see core/kinematics.build_redistribution_matrix_exact
    # and core/sigma_closed_form).
    p.add_argument("--kernel-nodes", default="linear_cos",
                   choices=["linear_cos", "exact_u"],
                   help="linear_cos: published linear cos(theta) grid with a hard "
                        "forward cut (column sums up to 1.9997 below ~1 GeV). "
                        "exact_u: analytic per-bin u-intervals, log-u Gauss-Legendre.")
    p.add_argument("--roi-recovery-model", default="cone",
                   choices=["cone", "template", "unity"],
                   help="cone: circular cone of --roi-half-angle (published). "
                        "template: w(theta) computed from the rho^2 template over "
                        "the real box ROI (halo). unity: w == 1, energy migration "
                        "only -- the correct IGRB treatment, since an isotropic "
                        "source against an isotropic scatterer has no boundary to "
                        "be deflected across.")
    p.add_argument("--emissivity-power", type=float, default=None,
                   help="Emissivity power p of the halo template that weights the "
                        "template ROI recovery w(theta). Default: read from "
                        "--halo-profile (rho2 -> 2, rho2.5 -> 2.5) when "
                        "--roi-recovery-model template, else 2. Recorded in the output.")
    p.add_argument("--sigma-model", default="linear_grid",
                   choices=["linear_grid", "closed_form"],
                   help="linear_grid: published quadrature. closed_form: analytic "
                        "sigma_tot, no angular grid.")
    p.add_argument("--x-roi", type=float, default=None,
                   help="1-cos(theta_ROI) for the hard-cut removal criterion; "
                        "only used with --sigma-model closed_form.")
    p.add_argument(
        "--f-bin", type=float, default=None,
        help="Override the energy-migration fraction f. Default (unset) derives "
             "it per bin from the loaded spectrum's own bin edges: 0.2305 on the "
             "halo grid (ratio 1.6890), 0.1591 on the IGRB grid (ratio 1.4142). "
             "Pass 0.231 to reproduce grids built before 2026-09-18, when f was "
             "hard-coded to the halo value for every dataset.")
    ...
    p.add_argument(
        ...
        default=None,
        help="Profile the test statistic over the source normalisation A "
             "(analytic weighted least squares). Default: on for --source pppc, "
             "off for --source measured/halo.",
    )
    p.add_argument(
    ...
    p.add_argument(
        "--drop-lowest-bins", type=int, default=0,
        help="Drop the N lowest-energy bins of the native grid before fitting. "
             ...
             "and energy-ceiling cuts.",
    )
    p.add_argument(
        "--e-max-fit", type=float, default=200.0,
        help="Photon-energy ceiling [GeV]. Bins above this are dropped from the "
             "fit for every dataset/source, and the EFT-validity omega_max is "
             "recomputed from the surviving bins. Default: no cut (use the "
             "dataset's full band).",
    )
...
def run_single(args) -> int:
    ...
    # source_spectrum() reports whether this source TYPE normally profiles over
    # the amplitude A: False for the measured/halo templates (the data are their
    # own source, so A is degenerate with the data at tau=0), True for PPPC.
    # That used to be a hard assignment, which meant the measured/halo grids
    # were always computed at A fixed to 1 with no way to ask for profiling.
    # It is now only the DEFAULT: --fit-normalization / --no-fit-normalization
    # override it explicitly. Leaving the flag unset reproduces the previous
    # behaviour exactly, for either source kind.
    if args.fit_normalization is None:
        args.fit_normalization = bool(source_fits_norm)
    else:
        args.fit_normalization = bool(args.fit_normalization)
        if args.fit_normalization != bool(source_fits_norm):
            print(f"  [override] fit_normalization={args.fit_normalization} "
                  f"(default for source={str(args.source).lower()!r} is {bool(source_fits_norm)})")
    ...
    # Bin geometry of THIS dataset. f_bin sets the removal threshold
    # u_E = f/(1-f) of the attenuation arm and is a property of the binning, so
    # it is derived from the retained bins unless --f-bin overrides it.
    from core.bin_geometry import bin_geometry_summary
    args.bin_geometry = bin_geometry_summary(E)
    _bg = args.bin_geometry
    if args.f_bin is None:
        print(f"  bin geometry   : ratio {_bg['bin_ratio_median']:.6f} "
              f"({'uniform' if _bg['uniform'] else 'non-uniform'}), "
              f"f_bin = {_bg['f_bin_median']:.6f} derived, "
              f"u_E = {_bg['f_bin_median'] / (1 - _bg['f_bin_median']):.6f}")
    else:
        print(f"  bin geometry   : ratio {_bg['bin_ratio_median']:.6f}, "
              f"f_bin = {float(args.f_bin):.6f} OVERRIDE "
              f"(derived value is {_bg['f_bin_median']:.6f})")

    validity_meta = validity_masks(args, E)

    # The template w(theta) is weighted by the halo emissivity, so it must use
    # the profile's own power. Before 2026-09-15 it was fixed at rho^2 for every
    # profile.
    if args.emissivity_power is None:
        if args.dataset == "halo" and args.roi_recovery_model == "template":
            from core.roi_recovery import emissivity_power_for_profile
            args.emissivity_power = emissivity_power_for_profile(args.halo_profile)
        else:
            args.emissivity_power = 2.0
    if args.roi_recovery_model == "template":
        print(f"  ROI recovery   : template w(theta), emissivity power {args.emissivity_power:g}")
    ...
    result = scan_reshaping_chi2(
        ...
        kernel_nodes=args.kernel_nodes,
        roi_recovery_model=args.roi_recovery_model,
        roi_emissivity_power=args.emissivity_power,
    ...
    # Template config for the removal-support clip below: one cfg carrying this
    # run's spectrum, operator and removal settings, with m_chi swapped per probe.
    clip_cfg = ReshapingConfig(
        m_chi=1.0, Lambda=1.0, dm_type=args.dm_type, operator=args.operator,
        c_s=args.c_s, c_p=args.c_p, c_phi=args.c_phi, majorana=args.majorana,
        E_bins=E, phi_0=source, phi_data=phi_data, phi_err=err,
        sigma_model=args.sigma_model, kernel_nodes=args.kernel_nodes,
        roi_recovery_model=args.roi_recovery_model,
        roi_emissivity_power=args.emissivity_power,
        tau_prefactor_override=spectrum.tau_prefactor_K,
        f_bin=args.f_bin, x_roi=args.x_roi,
        n_theta=args.n_theta, apply_roi_weight=not args.no_roi_weight,
        roi_half_angle_deg=args.roi_half_angle,
    )
    arm_of = {"raw_attenuation": "attenuation", "spectral_reshaping": "reshaping"}
    for model_kind, (chi_key, norm_key) in model_specs.items():
        ...
        # Clip where this arm removes nothing at all. Above the energy-migration
        # closure the removal cross section is identically zero at every Lambda,
        # so the contour the extractor draws there -- it interpolates linearly
        # across the mass cell containing the closure and overshoots it by up to
        # one cell -- is not a weak constraint but no constraint. The test is the
        # cross section itself, so it never fires on an arm that keeps angular
        # escape (the halo) or on the reshaping arm, which integrates the full
        # elastic cross section from u = 0.
        n_before = len(boundary)
        boundary, m_limit = clip_boundary_to_removal_support(
            boundary, clip_cfg, arm=arm_of[model_kind])
        if m_limit is not None:
            print(f"  removal closes : {model_kind} tau = 0 above "
                  f"m_chi = {m_limit:.2f} GeV; clipped {n_before - len(boundary)} "
                  f"of {n_before} contour points")
