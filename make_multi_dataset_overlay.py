...
from helpers.trinity_plotting import save_figure, set_paper_style, theme_ink as _ink, theme_legend_kw  # noqa: E402
...
# Per-dataset override. The IGRB arm was regenerated on the corrected Ackermann
# Table 3 spectrum (see docs/FINDING_2026-08-13_igrb_spectrum_not_table3.md) and
# therefore carries a different variant tag from the halo grids, which were not
# affected and must not be re-run. Without this the single global suffix would
# force both arms onto the same tag, and the only way to satisfy it would be to
# overwrite an existing .npz. Falls back to BOUNDARY_SUFFIX when unset.
DATASET_SUFFIX_OVERRIDE: dict[str, str] = {}


def _suffix_candidates() -> list[str]:
    """Every variant tag that might be worn by a boundary file this run."""
    seen, out = set(), []
    for t in [BOUNDARY_SUFFIX, *DATASET_SUFFIX_OVERRIDE.values()]:
        if t not in seen:
            seen.add(t); out.append(t)
    # Longest first: "_a_b" must be tried before "_a", or the shorter tag
    # strips the wrong number of characters and the filename grammar breaks.
...
def discover_boundaries(operator_key: str, sources: list[str],
    ...
    for path in sorted(BOUNDARY_DIR.iterdir()):
        ...
        for _tag in _suffix_candidates():
            tail = f"{_tag}_90cl.npz"
            if path.name.endswith(tail):
                break
        else:
            continue
        parsed = _parse_boundary_filename(
            path.name[: -len(tail)] + "_90cl.npz" if _tag else path.name)
        if parsed is None:
            continue
        # Honour the per-dataset override: a file wearing a tag that belongs to
        # a different dataset is not this dataset's grid.
        _want = DATASET_SUFFIX_OVERRIDE.get(parsed["dataset"], BOUNDARY_SUFFIX)
        if _tag != _want:
            continue
...
# Whether the partial-wave unitarity guide contributes to the EFT-valid floor.
# Mirrors make_sensitivity_map.DRAW_UNITARITY and
# make_paper_style_operator_overlays.DRAW_UNITARITY: True is the legacy
# standalone behaviour, and the master figure script sets it False so the
# published f2/f3/f4 all shade above the kinematic curve alone.
DRAW_UNITARITY = True
...
def plot_overlay_panel(ax, operator_key: str, records: list[dict],
    ...
    # Shade above the validity floor. The unitarity guide contributes only when
    # DRAW_UNITARITY is set, which is what f2 and f3 do.
    #
    # CORRECTED 2026-08-11: this maximum used to be taken unconditionally, with
    # a comment claiming it matched f2/f3. It did not -- both of those gate the
    # same maximum on DRAW_UNITARITY, which the master figure script sets False
    # (the legacy unitarity expressions are dimensionally inconsistent and the
    # curve was struck from f2/f3/f4 on 2026-08-06). The ungated maximum raised
    # the f4 Rayleigh Full wedge by up to 0.92 dex below m_chi = 2.6e-5 GeV, so
    # the same operator carried visibly different wedges in Figs. 3 and 4.
    _lam_floor = lam_kin
    if DRAW_UNITARITY:
        try:
            from make_sensitivity_map import _unitarity_curve
            _lu, _ = _unitarity_curve(operator_key, xg)
            if _lu is not None:
                _lam_floor = np.maximum(lam_kin, _lu)
        except Exception:
            pass
    ax.fill_between(xg, _lam_floor, np.full_like(_lam_floor, y_lim[1]),
