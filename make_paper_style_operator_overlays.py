...
from helpers.trinity_plotting import get_cmap_colors, save_figure, set_plot_style, set_paper_style, theme_ink as _ink, theme_legend_kw
...
DEFAULT_FERMI_SPECTRUM = Path(__file__).resolve().parent.parent / "fermi_data" / "york" / "processed" / "spectrum_data.txt"
...
DATA_DRIVEN_PROFILE_STYLES = {
    ...
    "pixelwise_global_rho2": {
        "label": r"This work, NFW $\rho^2$ halo",
        "raw_attenuation_label": r"This work, NFW $\rho^2$ halo",
        "color": "#111111",
        # Solid since the 2026-09-15 restyle. Dashed, it interleaved with the
        # dashed rho^2.5 curve drawn on top and the pair read as one two-tone
        # line. Solid underneath with the rho^2.5 dashes over it keeps both.
        "raw_ls": "-",
        "reshaping_ls": "-",
        "lw": 1.6,
    },
    "pixelwise_global_rho2.5": {
        "label": r"This work, NFW $\rho^{2.5}$ halo",
        "raw_attenuation_label": r"This work, NFW $\rho^{2.5}$ halo",
        "color": "#C92A2A",
        "raw_ls": (0, (4, 2)),
        "reshaping_ls": "-",
        "lw": 1.6,
...
DRAW_UNITARITY = True   # see make_sensitivity_map.DRAW_UNITARITY
BOUNDARY_SUFFIX = ""
...
def plot_panel(
    ...
    # 2026-07-17: the legacy Totani tension-scan boundary is OPTIONAL. For the
    # dipole operators the legacy files were produced with the wrong (direct-
    # detection) amplitudes and have been retired to _pre_fix_backup_2026-07-17;
    # the panel draws the corrected data-driven boundaries regardless.
    ...
    if lam_unit is not None and DRAW_UNITARITY and np.any((lam_unit >= y_min) & (lam_unit <= y_max)):
        h_unit, = ax.loglog(
            xgrid,
            lam_unit,
    ...
    if lam_unit is not None and DRAW_UNITARITY:
    ...
    if annotate_theory_guides and lam_unit is not None and DRAW_UNITARITY:
