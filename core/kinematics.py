...
def roi_recovery_fraction(
    ...
    """
    SUPERSEDED by core.roi_recovery.halo_recovery_fraction. Still callable, and
    still the default, so every published grid stays reproducible -- but it
    should not be used for new work.

    WHY IT IS SUPERSEDED
    --------------------
    This models the ROI as a CIRCULAR CONE of half-angle roi_half_angle_deg
    with the photon at the cone's centre. The ROI actually integrated is a box,
    |l| <= 60, 10 <= |b| <= 60, with a 20-deg-wide disk cut through the middle,
    and the halo template fills it rather than sitting at a point. The docstring
    below already conceded this ("assumes the photon originates at the ROI
    centre"); roi_recovery.compute_w_curve does the source-position average
    that fixes it, against the same rho^2 template used for the optical depth.

    The computed curve is not a cone of any radius:
    ...
    It falls immediately, because ~44% of the ROI solid angle lies within a few
    degrees of the sharp |b| = 10 deg edge that no cone represents, yet it keeps
    a long tail, because the ROI is 120 deg wide in longitude. Neither cone
    brackets it.

    Note also that this function takes cos(theta), whereas the replacement takes
    theta in DEGREES.
...
# ---------------------------------------------------------------------------
# Exact-interval redistribution kernel (log-u nodes)
# ---------------------------------------------------------------------------
# build_redistribution_matrix above carries two quadrature errors, both of which
# this replaces rather than merely refines:
#
# 1. NODES. It integrates on np.linspace(-1, cos_theta_max, n_theta), linear in
#    cos(theta), with n_theta = 1000. The forward peak sits at u ~ 1, i.e. at
#    x = 1 - cos(theta) ~ m_chi/omega, so resolving it with ten points needs
#    ~34,000 nodes at m_chi = 0.1 GeV and ~3.4e6 at 1e-3 GeV. Below those
#    counts the trapezoid rule returns peak height times grid spacing, which is
#    not convergent -- it scales as 1/n_theta.
#
# 2. FORWARD CUT. cos_theta_max = 1 - 1e-4 discards x < 1e-4 outright. For
#    m_chi < 1.7e-2 GeV the whole peak lies inside the discarded region.
#
# 3. BIN EDGES. Bin membership was a boolean mask, `(E_out >= E_lo) & (E_out <
#    E_hi)`, summed with np.gradient weights. The angular limits of each output
#    bin therefore snapped to whichever nodes happened to fall inside, so K
#    changed discontinuously as nodes crossed a bin edge.
#
# (3) does not need a finer grid at all -- it has an exact answer. Compton
# kinematics give E'/E = 1/(1+u) monotonically, so the photons from input bin j
# landing in output bin i occupy a CONTIGUOUS interval with analytic endpoints:
#
#     E' in [E_lo_i, E_hi_i]   <=>   u in [E_j/E_hi_i - 1,  E_j/E_lo_i - 1]
#
# clipped to the physical range [0, 2r]. Each K[i,j] is then integrated between
# exact limits, with no masking and no quantisation. Within each interval the
# integrand is smooth, so geometric segmentation plus Gauss-Legendre converges
# quickly even when the interval spans many decades in u.
