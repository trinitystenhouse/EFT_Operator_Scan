...
from core.attenuation_eft import (
    ...
    roi_tau_prefactor,
    # E_BINS_GEV / PHI_TOTANI / SIGMA_TOTANI are NOT imported here: they are
    # defaults for callers that pass no energy axis, and importing them by name
    # would defeat the lazy load in core.attenuation_eft (see the note there)
    # and make this module unimportable without the companion's chains.
    COS_THETA_MAX,
    FB_TO_CM2,
    OPERATOR_METADATA,
)
...
def _default_spectrum(i):
    """i-th of (E_BINS_GEV, PHI_TOTANI, SIGMA_TOTANI), loaded on demand."""
    for name in ("E_BINS_GEV", "PHI_TOTANI", "SIGMA_TOTANI")[i:i + 1]:
        if name in globals():                 # set by configure_halo_profile
            return globals()[name].copy()
    from core.attenuation_eft import totani_arrays
    return totani_arrays()[i].copy()
...
class ReshapingConfig:
    ...
    # Integration
    #
    # sigma_model selects how sigma_tot is obtained:
    #   "linear_grid" (default) -- LEGACY. Integrates dsigma/dOmega on a linear
    #       cos(theta) grid of n_theta nodes with a hard forward cut at
    #       cos_theta_max. CARRIES A QUADRATURE ERROR: the forward peak has
    #       angular width ~ m_chi/omega, so for omega/m_chi >~ 100 it falls
    #       between nodes and the trapezoid returns peak-height x spacing. The
    #       estimate is not convergent -- it scales as 1/n_theta and only
    #       stabilises once n_theta > 2*omega/m_chi (~3e5 at m_chi = 1e-3 GeV,
    #       against the 160-200 actually used). Measured against a resolved log
    #       grid at the SAME cut it is wrong by up to a factor 126 at
    #       m_chi <~ 0.1 GeV, in operator-dependent directions.
    #       RETAINED so the published grids can be regenerated. It reproduces
    #       the published numbers, which are affected by that error at low mass.
    #       It is NOT a defensible alternative to "closed_form".
    #   "closed_form" -- exact F_i(u_min) - F_i(u_max), no grid anywhere.
    #       Verified against the amplitude by symbolic differentiation and
    #       against resolved quadrature to 1e-6 from m_chi = 1e-5 to 1e9 GeV.
    sigma_model: str = "linear_grid"

    # Fractional energy loss defining bin migration. Only used by
    # sigma_model="closed_form".
    #   None (default) -- DERIVED PER BIN from E_bins by core.bin_geometry, so
    #       the threshold belongs to the dataset that is loaded. Gives 0.230548
    #       on the halo grid (ratio 1.689030) and 0.159104 on the IGRB grid
    #       (ratio 1.414214).
    #   float or (nE,) array -- explicit override, for reproducing grids built
    #       before 2026-09-18, when this field was a hard-coded 0.231 and every
    #       non-halo run silently used the halo's binning.
    f_bin: Optional[float | np.ndarray] = None
    # 1 - cos(theta_ROI) for the hard-cut removal model. None -> energy
    # migration only. Superseded by the computed w(theta) when
    # roi_recovery_model="template" (see core/roi_recovery.py).
    ...
    # Kernel integration nodes.
    #   "linear_cos" : np.linspace(-1, cos_theta_max, n_theta), the published
    #                  path. Carries three errors -- a linear grid that cannot
    #                  resolve a forward peak of width ~ m_chi/omega (needs
    #                  ~34k nodes at m_chi = 0.1 GeV against the 1000 used), a
    #                  hard forward cut discarding x < 1e-4 (the whole peak for
    #                  m_chi < 1.7e-2 GeV), and bin edges that snap to nodes.
    #   "exact_u"    : analytic per-bin u-intervals with log-spaced segmented
    #                  Gauss-Legendre. Column-sum closure holds to machine
    #                  precision and sigma_tot matches the closed forms to
    #                  1.8e-10 across 1e-5 to 1e8 GeV.
    # Default is unchanged so every published grid stays reproducible.
    kernel_nodes: str = "linear_cos"

    # ROI recovery weight applied inside the kernel.
    #   "cone"     : kinematics.roi_recovery_fraction, a circular cone of
    #                half-angle roi_half_angle_deg with the source at its
    #                centre. The published choice.
    #   "template" : roi_recovery.halo_recovery_fraction, computed from the
    #                halo template rho^p (p = roi_emissivity_power) over the
    #                actual box ROI. Not a cone at any radius: 0.895 at 5 deg
    #                where the 60 deg cone gives 1.0, but 0.15 at 90 deg where
    #                that cone gives 0 (values for p = 2).
    #   "unity"    : w == 1 identically -- the correct IGRB treatment. An
    #                isotropic source seen against a scatterer of the
    #                cosmological mean density has no boundary for a photon to
    #                be deflected across: every photon scattered out of a line
    #                of sight is replaced by a statistically identical one
    #                scattered in. Angular escape removes nothing, so ENERGY
    #                MIGRATION is the only channel. Declared explicitly rather
    #                than left implicit in "not template", because the halo and
    #                IGRB arms then have visibly different loss channels -- see
    #                roi_recovery.igrb_recovery_fraction for the derivation.
    # Only consulted when kernel_nodes="exact_u"; the legacy path keeps its own.
    roi_recovery_model: str = "cone"
    # Emissivity power p of the halo template that weights the "template" w(theta).
    # Must match the halo profile (2 for rho^2, 2.5 for rho^2.5). Before
    # 2026-09-15 it was fixed at 2 for every profile.
    roi_emissivity_power: float = 2.0

    # Energy / spectrum
    # Defaults resolve the halo spectrum lazily (core.attenuation_eft), so a
    # config that carries its own spectrum -- which is every config the figure
    # and constraint pipelines build -- never touches the companion's chains.
    E_bins: np.ndarray = field(default_factory=lambda: _default_spectrum(0))
    phi_0: np.ndarray = field(default_factory=lambda: _default_spectrum(1))
    phi_data: np.ndarray = field(default_factory=lambda: _default_spectrum(1))
    phi_err: np.ndarray = field(default_factory=lambda: _default_spectrum(2))
...
def resolve_f_bin(f_bin, E_bins) -> np.ndarray:
    """Energy-migration fraction to use: the override if given, else derived.

    Derivation is per bin from the bin edges of the spectrum actually loaded
    (core.bin_geometry). A scalar override is broadcast to every bin, which is
    what the pre-2026-09-18 hard-coded 0.231 did.
    """
    from core.bin_geometry import f_bin_per_bin
    E = np.asarray(E_bins, dtype=float)
    if f_bin is None:
        return f_bin_per_bin(E)
    f = np.asarray(f_bin, dtype=float)
    if f.ndim == 0:
        return np.full_like(E, float(f))
    if f.shape != E.shape:
        raise ValueError(f"f_bin has shape {f.shape}, expected scalar or {E.shape}")
    return f
...
def compute_tau_spectrum(cfg: ReshapingConfig, *, arm: str = "attenuation") -> np.ndarray:
    ...
    if getattr(cfg, "sigma_model", "linear_grid") == "closed_form":
        return _tau_closed_form(cfg, arm=arm)
    return _compute_tau_spectrum_linear_grid(cfg)
...
def _tau_closed_form(cfg: ReshapingConfig, *, arm: str) -> np.ndarray:
    ...
    kwargs = dict(c_s=cfg.c_s, c_p=cfg.c_p, c_phi=cfg.c_phi,
                  f_bin=resolve_f_bin(cfg.f_bin, E_bins))
    if arm == "reshaping":
        ...
        x_roi = getattr(cfg, "x_roi", None)
        if getattr(cfg, "roi_recovery_model", "cone") == "template":
            ...
            from core.roi_recovery import halo_recovery_callable
            sigma = sigma_removal_smooth_w_cm2(
                cfg.operator, cfg.m_chi, E_bins, cfg.Lambda,
                halo_recovery_callable(getattr(cfg, "roi_emissivity_power", 2.0)), **kwargs)
        else:
...
def removal_support_limit(cfg: ReshapingConfig, *, arm: str = "attenuation",
                          m_lo: float = 1e-6, m_hi: float = 1e8,
                          rtol: float = 1e-9) -> float | None:
    """Largest m_chi at which this arm removes anything at all, or None.

    Returns None when the arm's cross section is non-zero right up to ``m_hi``,
    which is the normal case: with angular escape in play (the halo arm) a photon
    can always be deflected out of the ROI, however small the energy shift, so
    nothing ever closes and there is nothing to clip.

    It returns a finite mass only where the removal cross section vanishes
    IDENTICALLY above it -- every retained bin, every Lambda. That happens when
    energy migration is the only channel and even a full backscatter leaves the
    photon inside its bin, u_E >= u_max = 2*omega/m_chi. Above that mass tau is
    exactly zero, the model is the unattenuated one at every Lambda, and a
    contour drawn there is not a weak constraint but no constraint.

    This is the physical condition, not a configuration string: it is read off
    the cross section the arm actually uses, so a dataset that loses angular
    escape by some other route is handled without a special case, and the halo
    arm is left alone by construction rather than by exemption.

    tau is evaluated at Lambda = 1. Whether tau is zero cannot depend on Lambda:
    tau scales as Lambda^(-p) with p = 4 or 6, which never changes a zero into a
    non-zero. Bisection is valid because the support is an interval from below --
    u_max = 2*omega/m_chi falls monotonically with mass while u_E is fixed, and
    the angular term r*x_roi falls with it, so once a bin closes it stays closed.
    """
    def has_removal(m):
        t = compute_tau_spectrum(_dc_replace(cfg, m_chi=float(m), Lambda=1.0), arm=arm)
        return bool(np.any(np.asarray(t, dtype=float) > 0.0))

    if has_removal(m_hi):
        return None
    if not has_removal(m_lo):
        return float(m_lo)
    lo, hi = float(m_lo), float(m_hi)
    while np.log(hi / lo) > rtol:
        mid = np.sqrt(lo * hi)
        if has_removal(mid):
            lo = mid
        else:
...
def clip_boundary_to_removal_support(boundary: np.ndarray, cfg: ReshapingConfig, *,
                                     arm: str = "attenuation") -> tuple[np.ndarray, float | None]:
    """Drop boundary points above the mass where this arm's removal vanishes.

    Returns (clipped boundary, the limiting mass or None). A no-op wherever the
    cross section never vanishes.
    """
    b = np.asarray(boundary, dtype=float)
    if b.size == 0:
        return b, None
    limit = removal_support_limit(cfg, arm=arm)
    if limit is None:
        return b, None
    return b[b[:, 0] <= limit], limit


def _compute_tau_spectrum_linear_grid(cfg: ReshapingConfig) -> np.ndarray:
    """
...
def _recovery_callable(cfg):
    ...
    if not cfg.apply_roi_weight:
        return None
    model = getattr(cfg, "roi_recovery_model", "cone")
    if model == "unity":
        return None                      # unity weights; see ReshapingConfig
    if model == "template":
        from core.roi_recovery import halo_recovery_callable
        return halo_recovery_callable(getattr(cfg, "roi_emissivity_power", 2.0))
    if model == "cone":
        if cfg.roi_half_angle_deg is None:
            return None
        # roi_recovery_fraction takes cos(theta), not degrees.
        return lambda th_deg: roi_recovery_fraction(
            np.cos(np.deg2rad(th_deg)), cfg.roi_half_angle_deg)
    raise ValueError("roi_recovery_model must be 'cone', 'template' or "
                     f"'unity', got {model!r}")


def _build_kernel_exact_u(cfg: ReshapingConfig) -> np.ndarray:
    """Kernel on exact per-bin u-intervals. See ReshapingConfig.kernel_nodes."""
...
def build_kernel(
    ...
    """
    if getattr(cfg, "kernel_nodes", "linear_cos") == "exact_u":
        return _build_kernel_exact_u(cfg)
    ...
    *,
    ...
    for component in components:
        ...
        if component.tau is None:
            tau = compute_tau_spectrum(comp_cfg, arm="reshaping")
        else:
...
def scan_reshaping_chi2(
    ...
    *,
    ...
    f_bin: float | np.ndarray | None = None,
    verbose: bool = True,
) -> dict:
    """
    ...
    # Resolved only if a caller omitted them; see core.attenuation_eft.
    if E_bins is None or phi_0 is None or phi_data is None or phi_err is None:
        from core.attenuation_eft import totani_arrays
        _dE, _dphi, _dsig = totani_arrays()
    _E = _dE if E_bins is None else np.asarray(E_bins, dtype=float)
    _phi0 = _dphi if phi_0 is None else np.asarray(phi_0, dtype=float)
    _phi_data = _dphi if phi_data is None else np.asarray(phi_data, dtype=float)
    _phi_err = _dsig if phi_err is None else np.asarray(phi_err, dtype=float)
    ...
    # Resolved once, from the energy axis actually being fitted, so every
    # (m_chi, Lambda) cell of one scan shares one bin geometry.
    _f_bin_resolved = resolve_f_bin(f_bin, _E)
    ...
    for i, m_chi in enumerate(m_chi_arr):
        for j, Lambda in enumerate(Lambda_arr):
            ...
            cfg = ReshapingConfig(
                ...
                roi_emissivity_power=roi_emissivity_power,
                sigma_model=sigma_model,
                x_roi=x_roi,
                f_bin=_f_bin_resolved,
            )
    ...
    result = {
        ...
        "sigma_model": sigma_model,
        "f_bin": _f_bin_resolved,
        "f_bin_mode": "derived" if f_bin is None else "override",
    }
    return result
