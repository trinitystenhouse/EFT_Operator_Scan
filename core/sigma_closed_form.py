...
r = 1e3, 1e5, 1e7, with unit constant of proportionality. See
UNITARITY_DERIVATION.md / the session log for the validation table.

WHY THIS EXISTS
---------------
The previous sigma_tot integrated dsigma/dOmega on a LINEAR cos(theta) grid of
160-200 nodes. The forward peak has angular width ~ m_chi/omega, so for
omega/m_chi > ~100 the peak falls between nodes and the trapezoid rule returns
the peak height times the grid spacing. That estimate is not convergent: it
scales as 1/n_theta and only stabilises once n_theta > 2*omega/m_chi (~3e5 at
m_chi = 1e-3 GeV, against the 160 actually used). Measured against a resolved
log grid at the SAME angular cut, the shipped sigma_tot is wrong by up to a
factor 126 at m_chi <~ 0.1 GeV, in operator-dependent directions.

These closed forms remove the quadrature from the problem entirely. The log-grid
quadrature is retained in the test suite as a CONSISTENCY CHECK validated
against these forms, not the reverse.
...
def sigma_removal_cm2(operator, m_chi, omega, Lambda, *,
                      ...
                      f_bin=0.231, x_roi=None, u_min=None,  # f_bin: halo-grid legacy default; callers pass the dataset's own, see core.bin_geometry
    ...
    if np.any(closed):
        if closed_channel == "raise":
            u_e = np.broadcast_to(np.asarray(f_bin, dtype=float) / (1.0 - np.asarray(f_bin, dtype=float)),
                                  np.shape(u_min))
            which = "energy migration (f/(1-f))" if np.all(u_min[closed] >= u_e[closed] - 1e-12) \
...
def sigma_removal_smooth_w_cm2(operator, m_chi, omega, Lambda, w_fn, *,
                               c_s=1.0, c_p=1.0, c_phi=1.0, f_bin=0.231,  # halo-grid legacy default; see core.bin_geometry
                               n_seg=24):
