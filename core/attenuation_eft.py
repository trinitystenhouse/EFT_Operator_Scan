...
# The halo spectrum is loaded LAZILY, not at import time (changed 2026-09-21).
#
# These three arrays are defaults for the standalone fitting scripts; nothing in
# the figure pipeline reads them, because every figure passes its own energy
# axis and fluxes explicitly. Loading them eagerly made importing this module --
# and therefore importing core.spectral_reshaping, and therefore building Figs.
# 3, 4 and 5 -- impossible without the companion paper's per-bin posterior
# chains, which the Zenodo deposit does not carry and the Data Availability
# statement attributes to that work. They are now resolved on first attribute
# access, so a consumer that actually needs them still gets the same arrays and
# the same loud FileNotFoundError if the chains are absent, and one that does
# not need them no longer pays for them.
_TOTANI_ARRAYS = None
_TOTANI_NAMES = ("E_BINS_GEV", "PHI_TOTANI", "SIGMA_TOTANI")


def totani_arrays():
    """(E_BINS_GEV, PHI_TOTANI, SIGMA_TOTANI), loaded once on first use."""
    global _TOTANI_ARRAYS
    if _TOTANI_ARRAYS is None:
        _TOTANI_ARRAYS = _load_totani_mcmc_arrays()
    return _TOTANI_ARRAYS


def __getattr__(name):            # PEP 562
    if name in _TOTANI_NAMES:
        return totani_arrays()[_TOTANI_NAMES.index(name)]
...
def configure_totani_arrays(mcmc_dir) -> None:
    ...
    """
    global E_BINS_GEV, PHI_TOTANI, SIGMA_TOTANI, _TOTANI_ARRAYS
    _TOTANI_ARRAYS = _load_totani_mcmc_arrays(Path(mcmc_dir))
    # Real module globals, which shadow the lazy __getattr__ above from here on.
    E_BINS_GEV, PHI_TOTANI, SIGMA_TOTANI = _TOTANI_ARRAYS
...
def compute_attenuated_spectrum(tau_bar, phi_totani=None):
    """Apply scattering attenuation: Phi_att(E) = Phi_Totani(E) * exp(-tau_bar(E)).

    phi_totani defaults to the halo posterior spectrum, resolved on demand so
    that importing this module does not require the companion's chains.
    """
    if phi_totani is None:
        phi_totani = totani_arrays()[1]
    return phi_totani * np.exp(-tau_bar)


def compute_chi2(tau_bar, phi_totani=None, sigma_totani=None):
    """Chi-squared between attenuated and observed halo flux."""
    if phi_totani is None or sigma_totani is None:
        _, _phi, _sig = totani_arrays()
        phi_totani = _phi if phi_totani is None else phi_totani
        sigma_totani = _sig if sigma_totani is None else sigma_totani
