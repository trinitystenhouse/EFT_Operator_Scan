...
def build_flat_counts_template(
    *,
    ...
    # Build a spatial-only flat template: uniform intensity inside the mask,
    # zero outside. Keep the normalization literal rather than rescaling by
    # the ROI mean, so the template matches the "flat with sharp edges"
    # construction described by Totani more closely.
    T = m.astype(float)
    if roi is not None:
        vals = T[roi]
        ...
        if not np.any(vals > 0):
            raise RuntimeError("Bubbles template has no non-zero pixels in ROI")
    ...
    *,
    ...
    for it in range(int(n_iter)):
        ...
        if bool(include_bubbles_in_fit) and it > 0:
            mu_pos_t = build_flat_counts_template(
                mask2d=pos_mask,
                roi2d=roi2d,
                expo=expo,
                omega=omega,
            ...
            mu_neg_t = - build_flat_counts_template(
                mask2d=neg_mask,
                roi2d=roi2d,
                expo=expo,
                omega=omega,
            ...
            templates_list.extend([mu_pos_t, mu_neg_t])
