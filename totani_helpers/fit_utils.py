...
def E2_from_pred_counts_maps(
    *, pred_counts_map, expo, omega, dE_mev, Ectr_mev, mask2d, tiny=1e-30
    ...
    mask2d: either (ny,nx) or (nE,ny,nx). The latter is used for
            energy-dependent pixel/source masks.
    ...
    mask_arr = np.asarray(mask2d, dtype=bool)
    if mask_arr.ndim == 2:
        mask3d = np.broadcast_to(mask_arr[None, :, :], pred_counts_map.shape)
    elif mask_arr.ndim == 3:
        if mask_arr.shape != pred_counts_map.shape:
            raise ValueError(f"mask shape {mask_arr.shape} != pred_counts_map shape {pred_counts_map.shape}")
        mask3d = mask_arr
    else:
        raise ValueError(f"mask must be 2D or 3D; got ndim={mask_arr.ndim}")
