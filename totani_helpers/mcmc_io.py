"""
I/O utilities for MCMC analysis results.

This module provides functions for loading, parsing, and processing MCMC output
files generated by the template fitting analysis. Handles coefficient extraction,
energy bin indexing, and flux scaling calculations.

Key Classes
-----------
MCMCCoeffTable : Container for MCMC coefficient tables across energy bins

Key Functions
-------------
load_mcmc_coeffs_from_dir : Load MCMC results from output directory
parse_energy_bin_from_filename : Extract energy bin index from filenames
add_flux_scaling_args : Add command-line arguments for flux calculations

The module supports both legacy (large) and compressed (minimal) MCMC output
formats, automatically detecting which fields are available.

Notes
-----
MCMC output files are expected to be `.npz` archives containing at minimum:
- 'labels': Component names
- 'coeffs_median': Median fitted coefficients
- 'coeffs_std': Standard deviations
"""
...
def add_flux_scaling_args(
    ...
    *,
    ...
    parser.add_argument(
        "--mcmc-stat",
        choices=["f_ml", "f_p05", "f_p16", "f_p50", "f_p84", "f_p95"],
        default="f_ml",
...
def pick_coeff(*, coeffs_by_label: Dict[str, np.ndarray], template_key: str) -> np.ndarray:
    ...
    psf_key = f"{template_key}_psf"
    if psf_key in coeffs_by_label:
        return coeffs_by_label[psf_key]
