...
STYLE_HINTS = {
    # Matched first: first key contained in the file stem wins, and the Lambiase
    # file name also contains "planck". Both digitised 2026-09-14.
    "lambiase": ("Planck energy injection (Lambiase+)", COSMOLOGY_COLOR, "-", "cosmology"),
    "pandax4t": ("PandaX-4T", "#E2A800", "-", "direct_detection"),
...
def _header_opts_out_of_autoplot(path):
    """True if the header carries '# autoplot: no'.

    For a digitisation kept here as a provenance record, or one that a figure
    script draws explicitly. Distinct from the untrustworthy markers above: the
    file is a valid digitisation, it is just not collected into every panel.
    """
    try:
        with open(path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if not s.startswith("#"):
                    break
                if s.lstrip("#").strip().replace(" ", "").startswith("autoplot:no"):
                    return True
    except OSError:
        return False
    return False
    ...
    for folder_name in spec["folders"]:
        ...
        for p in sorted(folder.glob("*.txt")):
            ...
            if _header_is_untrustworthy(p):
                ...
                continue
            if _header_opts_out_of_autoplot(p):
                continue
            paths.append(p)
